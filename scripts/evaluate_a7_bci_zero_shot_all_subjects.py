#!/usr/bin/env python3
"""A7 all-subject BCI zero-shot reconstruction audit."""
...
def plot_subject_points(subject_width: list[dict[str, Any]], output_dir: Path) -> list[Path]:
    ...
    for dataset in sorted({row["dataset"] for row in subject_width}):
        for metric in PRIMARY_METRICS:
            ...
            ax.set_title(f"A7 {dataset} subject-level zero-shot {metric}")
            ax.grid(True, color="#e5e7eb", linewidth=0.8)
            ax.legend(frameon=False)
            fig.tight_layout()
            path = plot_dir / f"a7_{dataset}_{metric}_subject_points.png"
            fig.savefig(path)
            plt.close(fig)
            paths.append(path)
    return paths
...
def write_markdown(
    ...
    *,
    ...
    lines = ["# A7 All-Subject BCI Zero-Shot Reconstruction", ""]
...
def main() -> None:
    ...
    write_csv(args.output_dir / "a7_channel_reconstruction_rows.csv", channel_rows)
    write_csv(args.output_dir / "a7_subject_checkpoint_rows.csv", subj_rows)
    write_csv(args.output_dir / "a7_subject_width_rows.csv", subj_width)
    write_csv(args.output_dir / "a7_subject_width_summary.csv", aggregate_stats)
    write_csv(args.output_dir / "a7_adjacent_width_subject_heterogeneity.csv", diminishing)
    write_csv(args.output_dir / "a7_dataset_window_pool_info.csv", dataset_info)
    summary = {
        "run_id": args.run_id,
        "protocol": {
            "target_policy": "BCI clean windows are assumed-clean/low-artifact windows, not guaranteed artifact-free neural ground truth.",
    ...
    (args.output_dir / "a7_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(
        args.output_dir / "a7_summary.md",
        run_id=args.run_id,
