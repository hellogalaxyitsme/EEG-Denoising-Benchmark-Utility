#!/usr/bin/env python3
"""A12 real-recording BCI IV-2a downstream analysis stratified by EOG energy."""
...
def write_figures(output_dir: Path, subject_aggregates: list[dict[str, Any]], inference: list[dict[str, Any]]) -> list[str]:
    ...
    try:
        ...
        (output_dir / "a12_figure_error.txt").write_text(f"matplotlib unavailable: {exc}\n", encoding="utf-8")
        return paths
    ...
    for base in base_values:
        ...
        path = fig_dir / f"a12_subject_pairs_base{base}.png"
    ...
    if den_inf:
        ...
        ax.grid(axis="y", color="#e5e7eb", linewidth=0.8)
        path = fig_dir / "a12_delta_by_eog_stratum.png"
        fig.savefig(path, dpi=200)
        plt.close(fig)
        paths.append(str(path))
    return paths
...
def write_markdown(
    ...
    lines = [
        "# A12 Real-Recording BCI IV-2a Downstream Analysis",
        "",
        "## Protocol",
        "",
...
def main() -> None:
    ...
    write_csv(args.output_dir / "a12_real_downstream_rows.csv", all_rows)
    write_csv(args.output_dir / "a12_real_downstream_subject_aggregates.csv", all_subject_aggregates)
    write_csv(args.output_dir / "a12_eog_stratum_rows.csv", all_stratum_rows)
    write_csv(args.output_dir / "a12_real_downstream_inference.csv", inference)
    write_csv(args.output_dir / "a12_quiet_heavy_contrasts.csv", contrasts)
    summary = {
        "info": {
            "run_id": args.run_id,
            "subjects": subjects,
    ...
    (args.output_dir / "a12_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(args.output_dir / "a12_summary.md", args, inference, contrasts, all_stratum_rows, figure_paths)
    print(f"[written] {args.output_dir / 'a12_summary.md'}", flush=True)
