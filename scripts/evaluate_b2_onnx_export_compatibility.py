#!/usr/bin/env python3
"""B2 ONNX export and numerical compatibility audit for compact denoisers."""
...
def write_markdown(path: Path, rows: list[dict[str, Any]], info: dict[str, Any], args: argparse.Namespace) -> None:
    lines = [
        "# B2 ONNX Export/Compatibility Audit",
        "",
        "## Scope",
        "",
...
def main() -> None:
    ...
    write_csv(args.output_dir / "b2_onnx_export_rows.csv", rows)
    write_csv(args.output_dir / "b2_onnx_export_failures.csv", failures)
    summary = {
        "info": {
            "run_id": args.run_id,
    ...
    (args.output_dir / "b2_onnx_export_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(args.output_dir / "b2_onnx_export_summary.md", rows, info, args)
    if failures:
        raise RuntimeError(f"{len(failures)} export(s) failed; see b2_onnx_export_failures.csv")
    print(f"[written] {args.output_dir / 'b2_onnx_export_summary.md'}", flush=True)
