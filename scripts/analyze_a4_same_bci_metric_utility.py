#!/usr/bin/env python3
"""A4 direct same-BCI reconstruction metric versus downstream utility analysis."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        required=True,
        help="One or more A3 result directories. Comma-separated entries are also accepted.",
    )
...
def build_metric_worklist(
    ...
    *,
    ...
    for source, rows in [("csp_lda", a2_rows), ("neural", a3_rows)]:
        for row in rows:
...
def compute_reconstruction_metric_rows(
    ...
    *,
    ...
    output_path = args.output_dir / "a4_bci_reconstruction_metric_rows.csv"
...
def aggregate_neural_utility_rows(a3_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
...
def write_markdown(path: Path, *, analysis: list[dict[str, Any]]) -> None:
    lines = ["# A4 Same-BCI Metric-Utility Analysis", ""]
...
def main() -> None:
    ...
    a2_rows = read_csv(args.a2_dir / "a1_checkpoint_seed_rows.csv")
    a3_dirs = expand_dirs(args.a3_dir)
    a3_rows = []
    for a3_dir in a3_dirs:
        a3_rows.extend(read_csv(a3_dir / "a3_checkpoint_classifier_seed_rows.csv"))
    worklist = build_metric_worklist(a2_rows, a3_rows, subjects=set(subjects), recipes=set(recipes))
    ...
    neural_utility = aggregate_neural_utility_rows(a3_rows)
    write_csv(args.output_dir / "a4_neural_utility_classifier_seed_aggregated_rows.csv", neural_utility)
    joined = build_joined_rows(a2_rows=a2_rows, neural_rows=neural_utility, metric_rows=metric_rows)
    write_csv(args.output_dir / "a4_metric_utility_joined_long_rows.csv", joined)
    analysis = analysis_rows(joined)
    write_csv(args.output_dir / "a4_subject_intercept_metric_tests.csv", analysis)
    summary = {
        "info": {
            "run_id": args.run_id,
    ...
    (args.output_dir / "a4_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(args.output_dir / "a4_summary.md", analysis=analysis)
    print(f"[written] {args.output_dir / 'a4_summary.md'}", flush=True)
