#!/usr/bin/env python3
"""Optimization/convergence audit for width-capacity experiments."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--current-eeg-prefix", required=True)
    parser.add_argument("--current-mixed-prefix", required=True)
    parser.add_argument("--extended-prefix", default="extended_optimization")
    ...
    for budget in budgets:
        ...
        for task in tasks:
            for metric, label in METRIC_SPECS:
                ...
                ax.set_title(f"optimization {budget} {task}: {label}")
                ax.set_xlabel("Epoch")
                ax.set_ylabel(label)
                ax.grid(True, color="#e5e7eb", linewidth=0.8)
                ax.legend(frameon=False, fontsize=8)
                fig.tight_layout()
                path = plot_dir / f"optimization_{budget}_{task}_{metric}.png"
                fig.savefig(path)
                plt.close(fig)
                paths.append(path)
    return paths
...
def write_markdown(
    ...
    *,
    ...
    lines = ["# Optimization/convergence audit", ""]
    ...
    lines.append("")
    lines.append("- Seed-mean curves with uncertainty ribbons summarize the matched trajectories.")
    lines.append("- Extended-budget rows provide an optimization sensitivity summary when available.")
...
def main() -> None:
    ...
    write_csv(args.output_dir / "optimization_epoch_history_rows.csv", epoch_rows)
    write_csv(args.output_dir / "optimization_seed_mean_curves.csv", curves)
    write_csv(args.output_dir / "optimization_run_selection_stability.csv", summary_rows)
    write_csv(args.output_dir / "optimization_capacity_optimization_comparisons.csv", comparisons)
    write_csv(args.output_dir / "optimization_budget_and_lr_sensitivity_deltas.csv", sensitivity)
    summary = {
        "run_id": args.run_id,
    ...
    (args.output_dir / "optimization_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(
