#!/usr/bin/env python3
"""A10 BCI IV-2a CSP+LDA control for spatial EOG contamination geometry."""
...
def write_markdown(path: Path, *, args: argparse.Namespace, inference: list[dict[str, Any]], contrasts: list[dict[str, Any]]) -> None:
    lines = ["# A10 Spatial EOG Projection CSP+LDA Robustness Control", ""]
...
def main() -> None:
    ...
    baseline_path = args.output_dir / "a10_baseline_seed_rows.csv"
    result_path = args.output_dir / "a10_checkpoint_seed_rows.csv"
    noise_path = args.output_dir / "a10_noise_projection_metadata_rows.csv"
    ...
    write_csv(args.output_dir / "a10_projection_coefficients.csv", projection_rows)
    write_csv(args.output_dir / "a10_subject_width_summary.csv", subject_rows)
    write_csv(args.output_dir / "a10_geometry_inference.csv", inference)
    write_csv(args.output_dir / "a10_geometry_contrasts.csv", contrasts)
    summary = {
        "info": {
            "run_id": args.run_id,
            "subjects": subjects,
    ...
    (args.output_dir / "a10_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(args.output_dir / "a10_summary.md", args=args, inference=inference, contrasts=contrasts)
    print(f"[written] {args.output_dir / 'a10_summary.md'}", flush=True)
