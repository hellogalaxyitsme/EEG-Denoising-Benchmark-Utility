#!/usr/bin/env python3
"""A3 neural-decoder classifier-seed uncertainty for BCI IV-2a downstream evaluation."""
...
def write_markdown(
    ...
    *,
    ...
    lines = ["# A3 Classifier-Seed Uncertainty Summary", ""]
    lines.append("CSP+LDA is deterministic once the input data and configuration are fixed; this A3 confirmatory run therefore targets neural-decoder initialization uncertainty.")
    lines.append("")
    ...
    lines.append("- Primary inference averages contamination, checkpoint, and classifier-seed repetitions within each subject before testing.")
...
def analyze_and_write(
    ...
    *,
    ...
    write_csv(args.output_dir / "a3_subject_primary_rows.csv", subject_rows)
    write_csv(args.output_dir / "a3_primary_subject_inference.csv", inference_rows)
    write_csv(args.output_dir / "a3_nuisance_variability.csv", variability_rows)
    write_csv(args.output_dir / "a3_variance_decomposition.csv", decomposition_rows)
    summary = {
        "info": {
            "run_id": args.run_id,
            ...
            "classifier_seeds": parse_int_list(args.classifier_seeds, name="classifier-seeds"),
    ...
    (args.output_dir / "a3_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(
        args.output_dir / "a3_summary.md",
        args=args,
        inference_rows=inference_rows,
        variability_rows=variability_rows,
        decomposition_rows=decomposition_rows,
    )
    print(f"[written] {args.output_dir / 'a3_summary.md'}", flush=True)
...
def main() -> None:
    ...
    baseline_path = args.output_dir / "a3_baseline_classifier_seed_rows.csv"
    result_path = args.output_dir / "a3_checkpoint_classifier_seed_rows.csv"
