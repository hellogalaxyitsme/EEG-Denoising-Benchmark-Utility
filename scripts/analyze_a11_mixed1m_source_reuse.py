...
def maybe_write_figures(output_dir: Path, counts_by_split_source: dict[tuple[str, str], np.ndarray]) -> list[str]:
    ...
    try:
        ...
        (output_dir / "a11_figure_error.txt").write_text(f"matplotlib unavailable: {exc}\n", encoding="utf-8")
        return figure_paths
    ...
    for split in SPLITS:
        ...
        path = fig_dir / f"a11_{split}_reuse_hist_ecdf.png"
...
def write_markdown(
    ...
    lines = [
        "# A11 Mixed-1M source-reuse/effective-sample-size audit",
        "",
    ...
    if figure_paths:
        ...
        lines.append("- Figure generation unavailable; use `a11_reuse_count_histogram.csv` and `a11_reuse_ecdf.csv`.")
    lines.append("")
...
def main() -> None:
    ...
    write_csv(args.output_dir / "a11_split_source_reuse_summary.csv", source_rows)
    write_csv(args.output_dir / "a11_recipe_source_reuse_summary.csv", recipe_rows)
    write_csv(args.output_dir / "a11_unique_combinations_summary.csv", combo_rows)
    write_csv(args.output_dir / "a11_split_source_overlap.csv", overlap)
    write_csv(args.output_dir / "a11_source_pool_split_sizes.csv", pool_rows)
    write_csv(args.output_dir / "a11_reuse_count_histogram.csv", hist_rows)
    write_csv(args.output_dir / "a11_reuse_ecdf.csv", ecdf)
    write_csv(args.output_dir / "a11_source_clustered_bootstrap.csv", bootstrap_rows)
    write_json(
        args.output_dir / "a11_summary.json",
        {
            "run_id": args.run_id,
            "data": str(args.data),
            "splits": splits,
    ...
    write_markdown(args.output_dir / "a11_summary.md", args, source_rows, overlap, bootstrap_rows, figure_paths)
