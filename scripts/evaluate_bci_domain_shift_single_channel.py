#!/usr/bin/env python3
"""domain-shift BCI IV-2a domain-adapted single-channel denoiser control."""
...
def load_multichannel_control_rows(path: Path | None, *, recipes: list[str]) -> list[dict[str, Any]]:
    if path is None:
        return []
...
def write_markdown(
    ...
    *,
    ...
    lines = ["# domain-shift BCI IV-2a Domain-Shift Control", ""]
...
def main() -> None:
    ...
    baseline_path = args.output_dir / "domain-shift_baseline_seed_rows.csv"
    result_path = args.output_dir / "domain-shift_domain_condition_seed_rows.csv"
    recon_path = args.output_dir / "domain-shift_reconstruction_seed_rows.csv"
    noise_path = args.output_dir / "domain-shift_noise_metadata_rows.csv"
    ...
    write_csv(args.output_dir / "domain-shift_subject_domain_summary.csv", subject_rows)
    write_csv(args.output_dir / "domain-shift_domain_inference.csv", inference)
    write_csv(args.output_dir / "domain-shift_adaptation_contrasts.csv", contrasts)
    write_csv(args.output_dir / "domain-shift_existing_multichannel_comparator.csv", multichannel_rows)
    summary = {
        "info": {
            "run_id": args.run_id,
            "subjects": subjects,
    ...
    (args.output_dir / "domain-shift_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(args.output_dir / "domain-shift_summary.md", args=args, inference=inference, contrasts=contrasts, multichannel_rows=multichannel_rows)
    print(f"[written] {args.output_dir / 'domain-shift_summary.md'}", flush=True)
