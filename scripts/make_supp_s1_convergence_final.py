"""Supplementary Figure S1: optimization and convergence."""
...
CURVES_CSV = ROOT / "results" / "manuscript_figure_data" / "supp_s1_convergence.csv"
HISTORY_CSV = ROOT / "results" / "manuscript_figure_data" / "supp_s1_seed_histories.csv"
OUT_DIR = ROOT / "results" / "released_plots"
DATA_DIR = ROOT / "results" / "manuscript_figure_data"
...
def load_seed_histories() -> pd.DataFrame:
    """Load individual trajectories for the two matched extended-training seeds."""
    history = pd.read_csv(HISTORY_CSV)
    history = history[
        history["budget"].eq("extended")
        & history["base"].isin(BASES)
        & np.isclose(history["lr"].astype(float), 0.001)
        & history["task"].isin([task for task, *_ in TASKS])
        & history["seed"].isin([42, 43])
    ].copy()
    if history.empty:
        raise ValueError("No matched extended-training seed histories found.")
    history["base"] = history["base"].astype(int)
    history["seed"] = history["seed"].astype(int)
    history["epoch"] = history["epoch"].astype(int)
    history["task_label"] = history["task"].map({task: label for task, label, *_ in TASKS})
    history["base_label"] = history["base"].map(BASE_LABELS)
    history["source_file"] = str(HISTORY_CSV.relative_to(ROOT))
    history["aggregation"] = "individual matched extended-training seed trajectory (no aggregation)"

    expected = {
        (task, base, seed): max_epoch
        for task, _, max_epoch, _ in TASKS
        for base in BASES
        for seed in (42, 43)
    }
    coverage = history.groupby(["task", "base", "seed"])["epoch"].agg(["min", "max", "count"])
    missing = [key for key, max_epoch in expected.items() if key not in coverage.index or tuple(coverage.loc[key]) != (1, max_epoch, max_epoch)]
    if missing:
        raise ValueError(f"Incomplete matched seed history coverage: {missing[:3]}")
    if history[["train_loss", "val_SDR"]].isna().any().any():
        raise ValueError("Matched seed histories contain missing plotted values.")
    return history.sort_values(["task", "base", "seed", "epoch"]).reset_index(drop=True)


def save_source_data(curves: pd.DataFrame, histories: pd.DataFrame) -> tuple[Path, Path]:
    ...
    history_out = DATA_DIR / "supp_s1_seed_histories.csv"
    history_columns = [
        "task", "task_label", "base", "base_label", "seed", "epoch", "train_loss", "val_SDR",
        "budget", "lr", "source_file", "aggregation",
    ]
    histories[history_columns].to_csv(history_out, index=False)
    return out, history_out


def draw_panel(ax, curves: pd.DataFrame, histories: pd.DataFrame, *, task: str, metric: str, max_epoch: int, original_budget: int) -> None:
    ...
    for base in BASES:
        ...
        history = histories[(histories["task"].eq(task)) & histories["base"].eq(base)]
        for _, seed_history in history.groupby("seed"):
            seed_history = seed_history.sort_values("epoch")
            ax.plot(
                seed_history["epoch"].to_numpy(dtype=float),
                seed_history[metric].to_numpy(dtype=float),
                color=color,
                alpha=0.28,
                linewidth=0.65,
                zorder=2,
        ...
        ax.fill_between(x, y - se, y + se, color=color, alpha=0.14, linewidth=0, zorder=3)
        ax.plot(x, y, color=color, linewidth=1.25, zorder=4)
...
def main() -> None:
    set_jne_style()
    curves = load_curves()
    histories = load_seed_histories()
    data_path, history_path = save_source_data(curves, histories)
    ...
    for col, (task, task_label, max_epoch, original_budget) in enumerate(TASKS):
        ...
        for row, (metric, metric_label) in enumerate(METRICS):
            ax = axes[row, col]
            draw_panel(ax, curves, histories, task=task, metric=metric, max_epoch=max_epoch, original_budget=original_budget)
            if row == 0:
    ...
    handles.extend([
        Line2D([0], [0], color="#4B5563", alpha=0.35, lw=0.7, label="Individual seed (n=2)"),
        Line2D([0], [0], color="#4B5563", lw=1.25, label=r"Mean; ribbon = $\pm$1 SE"),
    ...
    print(f"source file(s) used: {CURVES_CSV.relative_to(ROOT)}, {HISTORY_CSV.relative_to(ROOT)}")
    print(f"number of observations plotted: {len(curves)} curve summary rows and {len(histories)} individual seed-history rows")
    print("observations represent: two matched extended-training seed trajectories with stored means and +/-1 SE ribbons; no subject inference")
    print(f"source data saved: {data_path.relative_to(ROOT)}, {history_path.relative_to(ROOT)}")
