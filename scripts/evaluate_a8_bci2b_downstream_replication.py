#!/usr/bin/env python3
"""A8 BCI IV-2b downstream denoising replication with subject-level inference."""
...
def write_markdown(path: Path, *, args: argparse.Namespace, subjects: list[str], recipes: list[str], classifiers: list[str], width_rows: list[dict[str, Any]]) -> None:
    lines = ["# A8 BCI IV-2b Downstream Replication", ""]
...
def main() -> None:
    ...
    baseline_path = args.output_dir / "a8_bci2b_baseline_seed_rows.csv"
    result_path = args.output_dir / "a8_bci2b_checkpoint_seed_rows.csv"
    noise_path = args.output_dir / "a8_bci2b_noise_metadata_rows.csv"
    ...
    write_csv(args.output_dir / "a8_bci2b_subject_width_summary.csv", subject_rows)
    write_csv(args.output_dir / "a8_bci2b_width_inference.csv", width_rows)
    summary = {
        "info": {
            "run_id": args.run_id,
            "dataset": "BCI_IV2b",
    ...
    (args.output_dir / "a8_bci2b_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(
        args.output_dir / "a8_bci2b_summary.md",
        args=args,
        subjects=subjects,
        recipes=recipes,
        classifiers=classifiers,
    ...
    print(f"[written] {args.output_dir / 'a8_bci2b_summary.md'}", flush=True)
