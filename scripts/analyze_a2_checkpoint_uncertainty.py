#!/usr/bin/env python3
"""A2 checkpoint-uncertainty analysis for repeated BCI IV-2a CSP+LDA evaluations."""
...
def write_markdown(
    ...
    *,
    ...
    lines = ["# A2 Checkpoint-Uncertainty Summary", ""]
...
def main() -> None:
    ...
    baseline_rows = read_csv(args.input_dir / "a1_baseline_seed_rows.csv")
    checkpoint_rows = read_csv(args.input_dir / "a1_checkpoint_seed_rows.csv")
    ...
    write_csv(output_dir / "a2_subject_primary_rows.csv", subject_rows)
    write_csv(output_dir / "a2_primary_subject_inference.csv", inference_rows)
    write_csv(output_dir / "a2_checkpoint_contamination_variability.csv", variability_rows)
    write_csv(output_dir / "a2_variance_decomposition.csv", decomposition_rows)
    write_csv(output_dir / "a2_best_fixed_width_per_recipe.csv", best_rows)
    ...
    (output_dir / "a2_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(
        output_dir / "a2_summary.md",
        run_id=args.run_id,
    ...
    print(f"[written] {output_dir / 'a2_summary.md'}", flush=True)
