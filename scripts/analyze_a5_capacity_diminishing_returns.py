#!/usr/bin/env python3
"""A5 formal capacity/diminishing-return analysis for reconstruction metrics."""
...
def write_markdown(
    ...
    *,
    ...
    lines = ["# A5 Capacity And Diminishing-Return Analysis", ""]
...
def main() -> None:
    ...
    write_csv(args.output_dir / "a5_reconstruction_metric_rows.csv", width_rows + bci_rows)
    write_csv(args.output_dir / "a5_capacity_metric_aggregates.csv", aggregates)
    write_csv(args.output_dir / "a5_synthetic_adjacent_increment_tests.csv", synthetic_adjacent)
    write_csv(args.output_dir / "a5_synthetic_compact_vs_large_tests.csv", synthetic_compact)
    write_csv(args.output_dir / "a5_bci_zero_shot_adjacent_increment_tests.csv", bci_adjacent)
    write_csv(args.output_dir / "a5_bci_zero_shot_compact_vs_large_tests.csv", bci_compact)
    write_csv(args.output_dir / "a5_descriptive_elbows.csv", elbows)
    summary = {
        "info": {
            "run_id": args.run_id,
    ...
    (args.output_dir / "a5_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_markdown(
        args.output_dir / "a5_summary.md",
        run_id=args.run_id,
    ...
    print(f"[written] {args.output_dir / 'a5_summary.md'}", flush=True)
    print(
